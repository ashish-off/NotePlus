...
const NoteItem = ({ note }: notePropType) => {
  return (
    ...
    >
      <div>
        <h3 className="text-md sm:text-xl text-amber-50/95 font-semibold mb-4">
          {note.title.length > 30
            ? note.title.substring(0, 30) + " ..."
            : note.title}
        </h3>
        <pre className="font-[Roboto] text-xs sm:text-base font-semibold whitespace-pre-wrap break-words ">
